...
import { ArrowLeft, Check, Clock, LockKeyhole, MapPin, Search, Store } from "lucide-react";
...
export function AddStoreWizardModal({ open, vendorId, draftOwnerId, setupBusiness = false, onOpenChange, onCreated, onBusinessCreated, policyGate, onBeforeCreate }: AddStoreWizardModalProps) {
  ...
  // PINs are intentionally never written into the offline wizard draft.
  ...
  const hasValidStaffPIN = /^\d{4,6}$/.test(staffPIN);
  const hasConfirmedStaffPIN = hasValidStaffPIN && staffPIN === confirmStaffPIN;
  const totalSteps = setupBusiness ? 4 : 3;
  const wizardSteps = setupBusiness ? ["Location", "Hours", "Staff PIN", "Review"] : ["Location", "Hours", "Review"];
  ...
  const createStore = async () => {
    ...
    if (setupBusiness && !hasConfirmedStaffPIN) {
      toast.error("Set and confirm a 4–6 digit staff PIN for the first store.");
      setStep(3);
      return;
    }
    ...
    >
      <div className="flex min-h-[590px] max-h-[calc(92vh-6rem)] flex-col bg-gray-50">
        ...
        <div className="flex items-center justify-between gap-3 border-t border-gray-100 bg-white px-5 py-4 sm:px-7">
          ...
          {step < totalSteps ? <Button type="button" onClick={() => { if (step === 1 && !validLocation) { toast.error(setupBusiness ? "Business name, store name, address, city, and country are required." : "Store name, address, city, and country are required."); return; } if (step === 2 && !hasHours) { toast.error("Choose at least one open day."); return; } if (setupBusiness && step === 3 && !hasConfirmedStaffPIN) { toast.error("Enter matching 4–6 digit staff PINs."); return; } setStep((current) => current + 1); }} className="rounded-xl bg-printa-red text-white hover:bg-red-700">Continue</Button> : <Button type="button" onClick={() => void createStore()} disabled={isSaving} className="rounded-xl bg-printa-red text-white hover:bg-red-700">{isSaving ? (setupBusiness ? "Creating business..." : "Creating store...") : (setupBusiness ? "Create Business & Store" : "Create Store")}</Button>}
        </div>
      </div>
    </ResponsiveModal>
  );
